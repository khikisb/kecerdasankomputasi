...
with tab2:
    ...
    Glucose= st.slider('Berapa tingkat Glukosa dalam darah anda ? (Glucose)', 0, 199, 85)
    
    BloodPressure = st.slider('Berapa tekanan darah anda ? (BloodPressure)', 0, 122, 66)
    
    SkinThickness = st.slider('Berapa ketebalan kulit anda ? (SkinThickness) ', 0, 99, 29)
    
    Insulin = st.slider('Berapa tingkat insulin pada darah anda ? (Insulin)', 0, 846, 0)
    
    BMI = st.slider('Masukkan indeks tubuh anda ? (BMI)', 0.000 , 67.10, 26.60)

    DiabetesPedigreeFunction = st.slider('Berapa presentase diabetes anda ? (DiabetesPedigreeFunction)', 0.078, 2.42, 0.35)
    
    Age = st.slider('Berapa umur anda ? (Age)', 0, 81, 31)
